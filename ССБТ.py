...
parsed_page = ws.page_parser_from_file('ssbt.html')
document_links = list(ws.get_attribute_values_by_element_value(parsed_page, 'a', 'href', 'Система стандартов'))
...
document_names = [name.replace('"', '') for name in document_names]
document_names = [name.replace('/', '_') for name in document_names]
document_names = [name.replace(' Система стандартов безопасности труда (ССБТ).', '') for name in document_names]
failed_downloads = []
...
with open('ssbt_failed.txt', 'w', encoding = 'utf-8') as f_out:
