# Данная программа скачивает стандарты ЕСЗКС с сайта Техэксперт.
# http://docs.cntd.ru/
...
parsed_page = ws.page_parser_from_file('eszks.html')
document_links = list(ws.get_attribute_values_by_element_value(parsed_page, 'a', 'href', 'Единая система'))
...
document_names = [name.replace('"', '') for name in document_names]
document_names = [name.replace('/', '_') for name in document_names]
document_names = [name.replace(' Единая система защиты от коррозии и старения (ЕСЗКС).', '') for name in document_names]
failed_downloads = []
...
with open('eszks_failed.txt', 'w', encoding = 'utf-8') as f_out:
