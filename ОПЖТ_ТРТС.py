# Программа скачивает документы НП "ОПЖТ" с официального сайта (раздел "Регламенты таможенного союза"):
# http://opzt.ru/category/tehnicheskoe-regulirovanie/reglamenty-tamozhennogo-soyuza/
...
documents_url = main_url + '/category/tehnicheskoe-regulirovanie/reglamenty-tamozhennogo-soyuza/'
...
for doc_num in range(0, len(document_names)):
    filename = str(document_names[doc_num]).replace('"', '').replace('/', '_')
