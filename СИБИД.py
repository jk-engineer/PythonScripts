# Данная программа скачивает стандарты СИБИД с сайта Техэксперт.
# http://docs.cntd.ru/
...
parsed_page = ws.page_parser_from_file('sibid.html')
document_links = list(ws.get_attribute_values_by_element_value(parsed_page, 'a', 'href', 'СИБИД'))
...
document_names = [name.replace('"', '') for name in document_names]
document_names = [name.replace('/', '_') for name in document_names]
document_names = [name.replace(' СИБИД.', '') for name in document_names]
failed_downloads = []
...
with open('sibid_failed.txt', 'w', encoding = 'utf-8') as f_out:
