# Данная программа скачивает стандарты ССНТ с сайта Техэксперт.
# http://docs.cntd.ru/
...
parsed_page = ws.page_parser_from_file('ssnt.html')
document_links = list(ws.get_attribute_values_by_element_value(parsed_page, 'a', 'href', 'Надежность в технике'))
...
document_names = [name.replace('"', '') for name in document_names]
document_names = [name.replace('/', '_') for name in document_names]
document_names = [name.replace(' Надежность в технике (ССНТ).', '') for name in document_names]
failed_downloads = []
...
with open('ssnt_failed.txt', 'w', encoding = 'utf-8') as f_out:
