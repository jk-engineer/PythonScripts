# Данная программа скачивает стандарты СРПП с сайта Техэксперт.
# http://docs.cntd.ru/
...
parsed_page = ws.page_parser_from_file('srpp.html')
document_links = list(ws.get_attribute_values_by_element_value(parsed_page, 'a', 'href', 'Система разработки'))
...
document_names = [name.replace(' Система разработки и постановки продукции на производство (СРПП).', '') for name in document_names]
failed_downloads = []
...
with open('srpp_failed.txt', 'w', encoding = 'utf-8') as f_out:
